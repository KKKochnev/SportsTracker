...
void SportsTracker::setupUI()
{
    ...
    // 2. Страница матчей турнира
    QWidget *matchesPage = new QWidget();
    QHBoxLayout *matchesLayout = new QHBoxLayout(matchesPage);

    // Левая панель - матчи
    QWidget *matchesPanel = new QWidget();
    QVBoxLayout *matchesPanelLayout = new QVBoxLayout(matchesPanel);
    ...
    matchesPanelLayout->addWidget(matchesList);
    ...
    matchesPanelLayout->addWidget(backButton1, 0, Qt::AlignRight);
    matchesLayout->addWidget(matchesPanel, 40);
    ...
    matchesLayout->addWidget(standingsTable, 60);
    stackedWidget->addWidget(matchesPage);

    // 3. Страница статистики матча
